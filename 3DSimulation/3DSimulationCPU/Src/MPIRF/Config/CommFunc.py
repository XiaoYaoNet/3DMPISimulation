...
def HDF5Create(dir,message,imgdata,vphantom=None):

    dir=dir+".h5"
    fileHandle= h5py.File(dir, 'w')

    fileHandle['/uuid'] =0
    fileHandle['/version'] = 0
    fileHandle['/time'] =int(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))

    if vphantom is not None:
        fileHandle['/tracer/volume'] = vphantom

    fileHandle['/measurement/data'] =message['Measurement']['MeaSignal']
    fileHandle['/reconstruction/data']=imgdata
